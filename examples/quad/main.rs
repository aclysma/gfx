            ...
            winit::event::Event::WindowEvent { event, .. } => {
                match event {
                    ...
                    winit::event::WindowEvent::Resized(dims) => {
                        ...
                        renderer.resize_swapchain(dimensions);
                    }
                    _ => { }
                }
            }
...
impl<B> Renderer<B> where B: hal::Backend {
    ...
    fn resize_swapchain(&mut self, dimensions: Extent2D) {
        #[cfg(feature = "gl")]
        {
            // TODO: These lines are probably here for a reason?
            // However I dont know how to call them :/
            //let context = self.surface.get_context();
            //context.resize(dimensions.to_physical(context.window().hidpi_factor()));
        }
